...
function createItem(text, isChecked) {
    ...
    liEl.innerHTML = `
        <input type="checkbox"><span class="todo-item__description" data-bs-toggle="modal" data-bs-target="#exampleModal">${text}</span>
        <button onclick="removeToDo(event)" class="todo-item__delete">Видалити</button>
    `;
...
function modal() {
    ...
    if (exampleModal) {
        exampleModal.addEventListener('show.bs.modal', event => {
            ...
            let itemDescription = event.relatedTarget.innerText;

            modalTitle.textContent = itemDescription;
        })
    }
}
